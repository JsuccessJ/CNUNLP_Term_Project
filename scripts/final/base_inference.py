...
def batch_loop(instructions, batch_size, tokenizer, model):
    outputs = []
    for i in tqdm(range(0, 1000, batch_size)):  # len(instructions)
...
# 결과 저장
test_results = [{"summary": summary} for summary in summaries][:1000]

# 결과를 JSON Lines (JSONL) 파일로 저장
output_file_path = "/data/jaesunghwang/sum_termproject/results/test_summary_results1000_new.jsonl"
